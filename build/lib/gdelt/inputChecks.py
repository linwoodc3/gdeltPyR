...
def dateInputCheck(date, version):
    ...
    if isinstance(date, str):
        ...
        if version == 1 and parse(
                date).date() == datetime.datetime.now().date():
            raise ValueError(
                (
                    ...
                    np.datetime64(
                        datetime.datetime.now().date()) - np.timedelta64(1,
                                                                         'D'))
            )
        ...
        if datetime.datetime.now().hour <= 6 and parse(date).date() == (
                    ...
                    days=1)):
            raise BaseException(
                'GDELT 1.0 posts the latest daily update by 6AM EST.'
                'The next update will appear in {0}'.format(
                    str(datetime.datetime.combine(
                        datetime.datetime.now(), datetime.datetime.min.time()
                    ) + datetime.timedelta(hours=6, minutes=00,
                                           seconds=00) - datetime.datetime.now())))
        ...
        if len(date) == 1:
            ...
            try:
                ...
                elif parse("".join(date)) < parse('Feb 18 2015') and int(
                        version) != 1:
                    raise ValueError(
                        '''GDELT 2.0 only supports \'Feb 18 2015 - Present\'queries currently. Try another date.'''
                    )
            ...
            return "".join(date)
                    ...
                    isinstance(date, list) or isinstance(date, np.ndarray)):
            ...
            if bool(parse(date[0]) < parse(date[1])) == False:
                raise ValueError(
                    'Start date greater than or equal to end date. Check your entered date query.'
                )

            elif np.all(
                    np.logical_not(np.array(
                        list(map(parse, date))) > datetime.datetime.now())
            ) == False:
                raise ValueError(
                    "One of your dates is greater than the current date. Check your entered date query."
                )
            elif np.any(
                    np.logical_not(np.array(list(map(parse, date)
                                                 )) > parse(
                        "Feb 18 2015"))) == True and int(version) != 1:
                raise ValueError(
                    '''GDELT 2.0 only supports \'Feb 18 2015 - Present\'queries currently. Try another date.'''
                )
                ...
                if np.all(
                        np.logical_not(np.array(
                            list(map(lambda x: parse(x), date)),
                            dtype='datetime64[D]') >= np.datetime64(
                            datetime.datetime.now().date()))) == False:
                    raise ValueError(
                        '''You have today's date in your query for GDELT 1.0.  GDELT 1.0\'s most recent data'''
                        ''' is always the trailing day (i.e. {0}).  Please retry your query.'''.format(
                            np.datetime64(
                                datetime.datetime.now().date()) - np.timedelta64(
                                1, 'D'))
                    )
            ...
            if np.all(
                ...
                raise ValueError(
                    "One or more of your input date strings is greater than today's date. Check input."
                )
...
def tblCheck(dataframe, tbl):
    ...
    if tbl == 'events' or tbl == '' or tbl == 'mentions':
        resultsUrlList = dataframe[2][
            dataframe[2].str.contains('export|mentions')]
    elif tbl == 'gkg':
