    ...
    #################################
        ...
        if coverage and int(version) != 1:
            ...
            if len(converted.tolist()) >= (3 * 192):
                warnings.warn(
                    '''\n\nThis query will download {0} files, and likely exhaust your memory with possibly 10s of
        ...
        if isinstance(converted, list) is True:
            ...
            converted = np.where((parse(converted) >= parse('2013 04 01')),
                                 ...
                                 np.where((parse(converted) < parse('2006 01 01') and (int(version) == 1)),
                                          parse(converted).strftime('%Y%m%d%H%M%S')[:4],
                                          parse(converted).strftime('%Y%m%d%H%M%S')[:6])).tolist()
